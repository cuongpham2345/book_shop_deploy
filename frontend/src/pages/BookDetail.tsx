...
import { ShoppingCart, ArrowLeft, Star, Package, BookOpenCheck, Zap, X, MapPin, Phone, User, CreditCard, FileText, Check } from 'lucide-react'
...
function CheckoutModal({ book, qty, onClose }: { book: BookDetail; qty: number; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const cartItems = (cartRes.data as any).result?.items ?? []
      const cartItem = cartItems.find((i: any) => i.bookId === book.id)
      ...
      toast.error((err as any)?.response?.data?.message || 'Có lỗi xảy ra')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link to="/books" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 mb-6 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div className="space-y-5">
            <div>
              <Link to={`/books?categoryId=${book.categoryId}`} className="text-sm text-gray-400 hover:text-gray-700 transition-colors">
                {book.categoryName}
              </Link>
            ...
            {book.avgRating > 0 && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-500">{book.avgRating.toFixed(1)}</span>
              </div>
            )}
            ...
            <div className="flex items-end gap-3">
              ...
              {hasDiscount && (
                <span className="text-lg text-gray-400 line-through mb-0.5">{formatPrice(book.price)}</span>
              )}
            </div>
                ...
                <>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                      <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-3 py-2 text-gray-600 hover:bg-gray-50 transition-colors">−</button>
                      <span className="px-4 py-2 text-sm font-medium border-x border-gray-200">{qty}</span>
                      <button onClick={() => setQty(Math.min(book.stockQuantity, qty + 1))} className="px-3 py-2 text-gray-600 hover:bg-gray-50 transition-colors">+</button>
                    </div>
                  </div>
